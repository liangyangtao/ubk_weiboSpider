...
// }
	...
	function Q(az) {
		...
		for (z = 0; z < t.t - 1; ++z) {
			...
			if ((az[z + t.t] += t.am(z + 1, 2 * t[z], az, 2 * z + 1, aA, t.t
					- z - 1)) >= t.DV) {
				az[z + t.t] -= t.DV;
				az[z + t.t + 1] = 1
			}
	...
	function E(aH, aE, aD) {
		...
		while (--aK >= 0) {
			var aA = (aD[--aL] == az) ? this.DM : Math.floor(aD[aL] * aQ
					+ (aD[aL - 1] + aO) * aP);
	...
	function P(t) {
		...
		for ( var az = 0; az < this.m.t; ++az) {
			var z = t[az] & 32767;
			var aA = (z * this.mpl + (((z * this.mph + (t[az] >> 15) * this.mpl) & this.um) << 15))
					& t.DM;
			z = az + this.m.t;
...
function getPassEncoding(pubKey, serverTime, nonce,password) {
